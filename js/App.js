...
const projectId = "8ba0fab1a8134bcabd64e6337709c65a";
//Your api secret in ifura.io
const projectSecret = "vQJUw18+I7YFN5Pontcj4obYtxWgzxZtiZetQ2ov2g1oBhYW+Ttf5Q";
window.CONTRACT = {
  address: "0x58eE375d4222FB79A7ea8b6Df31e6032638FF831",
...
async function uploadFileToIpfs() {
  ...
  try {
    ...
    const response = await fetch("https://mainnet.infura.io/v3/8ba0fab1a8134bcabd64e6337709c65a", {
      method: "POST",
      body: formData,
      headers: {
        Authorization: auth,
...
async function sendHash() {
  ...
  try {
    ...
    if (!window.hashedfile || window.hashedfile.length < 4) {
      throw new Error("File hash not available. Please re-select the file.");
    }

    // Call the contract method
    const tx = window.contract.methods.addDocHash(window.hashedfile, CID);
    tx.send({ from: window.userAddress })
      .on("transactionHash", function (hash) {
        console.log("Transaction sent:", hash);
        $("#note").html(
          `<h5 class="text-info p-1 text-center">Waiting for confirmation...</h5>`
        );
      })
      .on("receipt", function (receipt) {
        console.log("Transaction confirmed:", receipt);
        printUploadInfo(receipt);  // Show hash, gas, block info
    ...
    console.error("Error in sendHash:", error.message);
    $("#note").html(`<h5 class="text-danger">${error.message}</h5>`);
    $("#loader").addClass("d-none");
    $("#upload_file_button").slideDown();
  }
